...
6. **`globals.css` Dosyasına Büyük CSS Blokları Gömme:** Yeni bir sayfa, bölüm veya karmaşık bileşene ait özel CSS kuralları doğrudan `globals.css` içerisine yazılamaz. Özel CSS blokları `src/styles/` altında modül olarak oluşturulmalı ve `globals.css` içerisinden `@import` ile dahil edilmelidir.
...
5. **Modüler CSS Mimarisi (`src/styles/`):** Bileşen veya sayfa düzeyinde özel CSS gerektiğinde (örneğin 3D sahneler, sticky timeline'lar, karmaşık clip-path animasyonları) dosyayı `src/styles/<modul-adi>.css` olarak oluşturup `src/app/globals.css` içerisine `@import "../styles/<modul-adi>.css";` ile ekle.
...
## 5. İleri Düzey Mimari ve Güvenlik (Advanced Best Practices)

### A. Server/Client Component İzolasyonu (Performans)
Next.js App Router mimarisinde veri çekme (data fetching) ve interaktivitenin doğru ayrılması zorunludur.
- **Kural:** Mümkün olan her yerde Server Components kullan. `"use client"` direktifini sayfanın tamamına (`page.tsx`) eklemek yerine, sadece etkileşim gerektiren (Framer Motion, `useState`, `onClick` vb.) en küçük alt bileşenlere (örneğin bir buton veya slider bileşenine) ekle. Veri çekme işlemleri her zaman Server Component tarafında (Sanity GROQ ile) yapılmalıdır.

### B. Çevresel Değişkenler (Güvenlik)
- **Kural:** Sanity token'ları (örneğin `SANITY_API_READ_TOKEN`) gibi gizli anahtarları asla istemci tarafına (Client Components) sızdırma. İstemci tarafında yalnızca `NEXT_PUBLIC_` ön eki olan güvenli anahtarlar kullanılabilir. Token gerektiren sorgular kesinlikle Server Component veya API Route (Route Handler) içerisinde yapılmalıdır.

### C. Hata Yönetimi ve Yükleme Durumları (UX/UI)
- **Kural:** Veri çekilen her dinamik rota için mutlaka Next.js App Router standartlarına uygun bir `loading.tsx` (skeleton loader veya spinner) ve `error.tsx` (şık bir hata ekranı) oluştur. Kullanıcı asla ham sunucu hataları veya boş beyaz ekranlarla karşılaşmamalıdır.

### D. Erişilebilirlik ve Animasyonlar (Framer Motion & a11y)
- **Kural:** Framer Motion ile karmaşık animasyonlar yaparken her zaman kullanıcı tercihlerine saygı duy. `useReducedMotion` kancasını (hook) kullanarak cihazında animasyonları kapatmış kullanıcılar için sadeleştirilmiş veya anında gerçekleşen geçişler (transitions) sun.

### E. Tailwind CSS v4 Standartları
- **Kural:** Tailwind v4'ün mimarisine uygun olarak, karmaşık tema yapılandırmalarını (renk paletleri, fontlar vb.) doğrudan modern CSS değişkenleri ve `@theme` direktifi ile yönet. Bileşen içindeki çok uzun utility class'larını okunabilirlik açısından gerekirse `clsx` ve `tailwind-merge` kullanarak düzenle.
